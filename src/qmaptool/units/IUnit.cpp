...
const QRegularExpression IUnit::reCoord1(QRegularExpression::anchoredPattern("^\\s*([N|S]){1}\\W*([0-9]+)\\W*([0-9]+\\.[0-9]+)\\s+([E|W|O]){1}\\W*([0-9]+)\\W*([0-9]+\\.[0-9]+)\\s*$"));

const QRegularExpression IUnit::reCoord2(QRegularExpression::anchoredPattern("^\\s*([N|S]){1}\\s*([0-9]+\\.[0-9]+)\\W*\\s+([E|W|O]){1}\\s*([0-9]+\\.[0-9]+)\\W*\\s*$"));

const QRegularExpression IUnit::reCoord3(QRegularExpression::anchoredPattern("^\\s*([-0-9]+\\.[0-9]+)\\s+([-0-9]+\\.[0-9]+)\\s*$"));

const QRegularExpression IUnit::reCoord4(QRegularExpression::anchoredPattern("^\\s*([N|S]){1}\\s*([0-9]+)\\W+([0-9]+)\\W+([0-9]+\\.[0-9]+)\\W*([E|W|O]){1}\\W*([0-9]+)\\W+([0-9]+)\\W+([0-9]+\\.[0-9]+)\\W*\\s*$"));

const QRegularExpression IUnit::reCoord5(QRegularExpression::anchoredPattern("^\\s*([-0-9]+\\.[0-9]+)([N|S])\\s+([-0-9]+\\.[0-9]+)([W|E])\\s*$"));
...
QDateTime IUnit::parseTimestamp(const QString& timetext, int& tzoffset)
{
    ...
    else if (QRegularExpressionMatchIterator it = tzRE.globalMatch(timetext); it.hasNext())
    {
        ...
        int offsetHours(timetext.mid(j + 1, 2).toUInt());
        int offsetMinutes(timetext.mid(j + 4, 2).toUInt());
        if (timetext[i] == '-')
        {
...
{
    QRegularExpressionMatch match = reCoord2.match(str);
    if(match.hasMatch())
    {
        bool signLat = (match.capturedView(1).compare("S") == 0);
        qreal absLat = match.capturedView(2).toDouble();
        lat = signLat ? -absLat : absLat;

        bool signLon = (match.capturedView(3).compare("W") == 0);
        qreal absLon = match.capturedView(4).toDouble();
        lon = signLon ? -absLon : absLon;
    }
    else if(QRegularExpressionMatch match2 = reCoord1.match(str); match2.hasMatch())
    {
        bool signLat = (match2.capturedView(1).compare("S") == 0);
        int degLat = match2.capturedView(2).toInt();
        qreal minLat = match2.capturedView(3).toDouble();

        GPS_Math_DegMin_To_Deg(signLat, degLat, minLat, lat);

        bool signLon = (match2.capturedView(4).compare("W") == 0);
        int degLon = match2.capturedView(5).toInt();
        qreal minLon = match2.capturedView(6).toDouble();
